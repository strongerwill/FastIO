...
First, such security patches exacerbate the problem that the execution paths of the guest page table (de)allocation become extremely long, which would consequently increase the latencies of the creations and exits of processes.
Second, the security validations introduce many additional IOTLB flushes. They lead to extra IOTLB misses, and the misses would introduce negative impacts for the I/O performance of all peripheral devices.


In this paper, we propose \name, a novel lightweight approach for improving the performance in page table management.
First, \name shortens the execution paths of the page table (de)allocations by a page table cache, which maintains a dedicated buffer for serving page table (de)allocations.
...
The micro experiment results indicate that \name is able to \emph{completely eliminate} the additional IOTLB flushes in the most of the time, and effectively reduces (de)allocation time of the page table by 47\% on average.
