...
In this section, we focus on describing the two identified performance issues.
First, we describe the critical steps of the long execution paths of the page table (de)allocation, and then describe the dependency between the page table (de)allocation and IOTLB flushes.

As Xen~\cite{XEN-SOSP03} is a typical and popular paravirtual hypervisor, in this section we use Xen in a x86 MMU model~\cite{x86-pv-model} to illustrate the details. We believe you can easily find the corresponding mechanisms on other paravirtual hypervisors.
If you are already familiar with these, you can skip this section to the motivation section (Section~\ref{sec:moti}).
...
\subsection{Long Execution Path of Page Table (De)allocation}\label{sec:exe-path}
...
\mypara{Page Table Validations}\label{sec:pv-security}
...
\mypara{DMA Validations}

\subsection{Addition IOTLB Flushes}
