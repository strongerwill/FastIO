...
Before we describe the design overview, I'd like to share the requirements we considered in the design.
...
\item (R2) Compatible with legacy applications. The new scheme should limit the modification on the guest kernel and the hypervisor, without any modifications on the existing applications.
...
flushes, as well as achieving the above three requirements.
...
This approach retain the system security, but it could not eliminate all IOTLB flushes, only reducing the number of IOTLB flushes in a certain level.
...
In addition, the creations of the page table also do not need IOTLB flush as the guest kernel could allocate pages from the cached pool, without allocating pages from the slow and inefficient buddy system. 
