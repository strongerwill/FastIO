...
In fact, the effects of the IOTLB misses are measurable, but it relies on the extremely high-speed I/O device, e.g., the one in Intel's I/O Acceleration Technology~\cite{lauritzenintel}, or the newly designed IOMMU, e.g., rIOMMU project~\cite{malka2015riommu} establishes a high-performance setting with the newly designed IOMMU and the ibverbs library~\cite{ibverbsevaluation,kerr2011dissecting}.
