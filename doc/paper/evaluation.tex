...
As shown in Figure~\ref{fig:PGDtime}, \ref{fig:PMDtime}, and \ref{fig:PTEtime}, there are (34\%, 47\%), (38\%, 22\%) and (65\%, 65\%) improvements for a pair of allocation and deallocation for three-level page table, from top to bottom.
%putting together
Putting them together, the page table allocations and deallocation can be improved by 45\% and 55\% on average respectively.
Note that the \name in the \dynname groups can achieve the same performance improvement as the one in the \prename group.
...
As mentioned before, the guest kernel firstly goes through the \cache to allocate page-table pages. If the cached semi-writable pages cannot satisfy the requirements (a.k.a, cache miss), the \cache would have to allocate writable pages using the existing memory allocators following the traditional paths,.
During this period, the execution time is the traditional execution path plus the path in the \cache.
As a result, the execution time of the page table allocation is even longer than the baseline (Figure~\ref{fig:overview}).
However, the overhead in this condition is not big, due to the list is empty and the control flow will immediately return.
In fact, the overhead is only on the extra function innovation, stack adjustment and the checking of the cache list and the page type.
Putting them together, the overhead are less than 20 instructions.
Fortunately, the worse case does not often occur. According to our observations in the \dynname group, the number of the worst cases is $348$, out of $198990$ allocation requests in 30 minutes.
Note that the page-table deallocation is always successful, finishing in a constant time.
...
\end{tabular}
\end{center}
\caption{The memory usage of the \cache is small in the \dynname group, only occupying 160 pages. }
\label{tab:dynPGpool}
\end{table}

\subsubsection{Memory Usage Measurement}
Besides CPU usage, we also measured the memory consumption of the \cache. 
To clearly see the page usage in different levels, we measured the numbers of the semi-writable pages and the page-table pages in three levels. 
The measurement results are listed in Table \ref{tab:prePGpool}.
In the \prename group the \cache has $5$, $26$ and $145$ semi-writable pages ready for allocations, totally occupying memory $704KB$.
In addition, it has already popped out $193$ page-table pages for serving 4 user processes. Roughly each process needs $48.25$ page-table pages.
The conditions is quite similar In the \dynname group, where the cache has $160$ semi-writable pages ready for allocations, totally occupying memory $640KB$.
In the both groups, the \cache consumes an insignificant memory usage, always less $1MB$. 
...
The purpose of the macro-benchmarks are to evaluate the effects of \name on the overall system. 
...
Lmbench is the macro-benchmark tool for measuring the latency of the process creations and exits (i.e., fork+exit, fork+execve, fork+/bin/sh -c), shown in Figure~\ref{fig:lmbench}. 
...
We believe this improvement will significantly benefit the workloads that rely on many short-time temporary processes.
...
All measurement results are list in Figure~\ref{fig:spec}. 
...
\subsubsection{I/O Performance Measurments}
...
However,  rIOMMU~\cite{malka2015riommu} claims that the overhead caused by walking IOMMU page tables due to IOTLB misses is so negligible that cannot be measured in the \emph{netperf}, because the main latency induced by I/O interrupt processing and the TCP/IP stack is several orders of magnitude larger than that of walking the page tables.
In addition, Nadav Amit \emph{et al.}~\cite{amit2012iommu} also has the similar statements.
...
The client on one machine set a bulk of TCP packets to the server on another machine. Note that emulated workload is also enabled on the client machine to trigger IOTLB flushes and misses.
...
Similarly, we also test the disk I/O using lmbench. The results indicate that the disk I/O speed is kept the same.
The above two experiments also as the new evidences to support the observations in \cite{amit2012iommu}  and  \cite{malka2015riommu}.
...
\end{tabular}
\end{center}
\caption{The netperf results for network I/O. The results indicate the overhead introduced by the IOTLB misses is negligible.}
\label{tab:netperf}
\end{table}

In fact, we can measure the overhead introduced by the IOTLB misses using a a high-speed I/O device (i.e., Intel's I/O Acceleration Technology~\cite{lauritzenintel}) to do DMA copy operations in a pseudo pass-through mode of IOMMU, as suggested in \cite{amit2012iommu}. 
In rIOMMU project, the authors make use of ibverbs library~\cite{ibverbsevaluation,kerr2011dissecting} to measure the cost of an IOTLB miss in the above setting.
The results indicate that DMA-copy speed is largely reduced by the IOTLB misses.
We believe that the \name can effectively reduce the overhead in the above high-speed setting, and we also plan to do the experiment in the future.
