...
The micro-benchmark measurements are to evaluate the frequency of the IOTLB flushes, as well as the execution time of the page table (de)allocations and the memory usage of the \cache.
...
In the baseline group, the frequency of the IOTLB flushes is increasing in the first five minutes, and then keeps at a high flush rate (i.e., 9050 flushes per minute) until the test finishes.
In the \prename group, the flush frequency quickly decreases from a low level (i.e., 332 flushes per minute) to zero level in about one minute and keeps at the level.
...
۪As there are three levels of the guest page table, and each level has its (de)allocation functions, e.g., pgd\_alloc and pgd\_free for the L1 (top) level.
...
That is why the measurement results of the \dynname group in the beginning 5 minutes are almost overlapped with the baseline results.
...
The only different is that the \name in the \dynname group need a 2 minutes transitional stage to be stable.

\mypara{The Worst Case}
When the \name starts to work, the guest kernel always first invoke the \cache for the page table allocations. 
If the cached semi-writable pages cannot satisfy the requirements (a.k.a, cache miss), the \cache would have to allocate writable pages form the existing memory allocators following the traditional paths,.
...
More specifically, the overhead consists of the function invocation, the stack adjustments and the checking of the cache list and the page type.
...
We also measured the memory consumption of the \cache.
...
\caption{The latencies on average is reduced by 11\%, 17\% and 21\%, from left to right.}
\label{fig:lmbench}
\end{figure}
...
In this paper, we test the network I/O and disk I/O performances under regular circumstances, and use these experiments to revisit of the problem between the IOTLB misses and I/O performance.
...
By comparing both values of $\mu$ and $\sigma$, we find that there is no detectable improments on the network I/O.
...
To evaluate the benefits of the \name on the IOTLB miss reduction, we need a extremely high-speed I/O device, like the one in Intel's I/O Acceleration Technology~\cite{lauritzenintel}.
In fact, in rIOMMU project, the authors has established a high-performance setting with the ibverbs library~\cite{ibverbsevaluation,kerr2011dissecting} to measure the cost of one IOTLB miss.
The experiments results indicate that the DMA-copy speed is largely reduced by the IOTLB misses.
...
Moshe et al. use rIOMMU with ibverbs library~\cite{ibverbsevaluation,kerr2011dissecting} to establish a high-performance setting in order to measure the cost of one IOTLB miss.
