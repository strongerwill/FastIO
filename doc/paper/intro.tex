...
First, \name shortens the length of the execution path of the page table (de)allocations by \cache, which maintains a dedicated buffer for serving page table (de)allocations (Figure~\ref{fig:overview} (b)).
The \cache queues the released/freed page-table page in the hope that
...
As the functionalities of the \cache are concentrated, it is relatively easy to make it small, simple and efficient, dramatically reducing the execution path.
...
Interestingly, the semi-writable pages can be smoothly maintained by the \cache with very small extra cost.
...
There are (34\%, 47\%), (38\%, 22\%) and (65\%, 65\%) improvements for a pair of allocation and deallocation for three-level page table, from top to bottom. In certain worst cases where there exists many page-table allocation requests with only a few or no deallocations, the additional performance overhead for page table (de)allocation in \name is less than 20 instructions compared to that of the baseline. According to the experiment, the number of worst cases is only $348$, out of the total allocation requests (i.e., $198990$), thereby imposing negligible impacts on \name.  
