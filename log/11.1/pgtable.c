...
void free_pgd_page(unsigned long addr)
{
    ...
    { 
    	...
    	if((pgd_used_counter/pgd_free_counter < 1) && ((pgd_used_counter + pgd_free_counter) >= 10))
...
void free_pmd_page(unsigned long addr)
{
	...
	//if((pmd_free_counter/pmd_used_counter>=3) && ((pmd_used_counter + pmd_free_counter) >= 1800))
    	...
    	if((pmd_used_counter/pmd_free_counter < 1) && ((pmd_used_counter + pmd_free_counter) >= 40))
...
void free_pte_page(struct page *pte)
{
    ...
    { 
    	...
    	if((pte_used_counter/pte_free_counter < 1) && ((pte_used_counter + pte_free_counter) >= 300))
	...
	{	
	  ...
	  {	
		...
		else
		{
			...
			if((unsigned int)act_time>=tm_incret*10*1000)
			{
				spin_lock(&pgd_alloc_cnt_lock);
