...
DEFINE_SPINLOCK(buddy_pgd_lock);
DEFINE_SPINLOCK(cache_pgd_lock);
DEFINE_SPINLOCK(buddy_pmd_lock);
DEFINE_SPINLOCK(cache_pmd_lock);
DEFINE_SPINLOCK(buddy_pte_lock);
DEFINE_SPINLOCK(cache_pte_lock);
long buddy_pgd_cnt = 0;
long cache_pgd_cnt = 0;
long buddy_pmd_cnt = 0;
long cache_pmd_cnt = 0;
long buddy_pte_cnt = 0;
long cache_pte_cnt = 0;
...
unsigned long alloc_pgd_page(void)
{
    ...
    if(pgd_head)
    {
        spin_lock(&cache_pgd_lock);
	cache_pgd_cnt++;
        spin_unlock(&cache_pgd_lock);
    ...
    {   
        spin_lock(&buddy_pgd_lock);
 	buddy_pgd_cnt++ ;
        spin_unlock(&buddy_pgd_lock);
...
unsigned long alloc_pmd_page(void)
{
    ...
    if(pmd_head)
    {
        spin_lock(&cache_pmd_lock);
	cache_pmd_cnt++;
        spin_unlock(&cache_pmd_lock);
    ...
    {   
        spin_lock(&buddy_pmd_lock);
 	buddy_pmd_cnt++;
        spin_unlock(&buddy_pmd_lock);
...
struct page *alloc_pte_page(void)
{
    ...
    if(pte_head)
    {
        spin_lock(&cache_pte_lock);
 	cache_pte_cnt++;
        spin_unlock(&cache_pte_lock);
    ...
    {   
        spin_lock(&buddy_pte_lock);
 	buddy_pte_cnt++;
        spin_unlock(&buddy_pte_lock);
	   ...
	   {	         	 
		...
		else
		{
			...
			if((unsigned int)act_time>=tm_incret*60*1000)
			{
				...
				spin_lock(&buddy_pgd_lock);
        			printk("buddy pgd invocation: %ld\n", buddy_pgd_cnt);
				buddy_pgd_cnt=0;
				spin_unlock(&buddy_pgd_lock);
				
				spin_lock(&cache_pgd_lock);
        			printk("cache pgd invocation: %ld\n", cache_pgd_cnt);
				cache_pgd_cnt=0;
				spin_unlock(&cache_pgd_lock);
				
				spin_lock(&buddy_pmd_lock);
        			printk("buddy pmd invocation: %ld\n", buddy_pmd_cnt);
				buddy_pmd_cnt=0;
				spin_unlock(&buddy_pmd_lock);
				
				spin_lock(&cache_pmd_lock);
        			printk("cache pmd invocation: %ld\n", cache_pmd_cnt);
				cache_pmd_cnt=0;
				spin_unlock(&cache_pmd_lock);
								
				spin_lock(&buddy_pte_lock);
        			printk("buddy pte invocation: %ld\n", buddy_pte_cnt);
				buddy_pte_cnt=0;
				spin_unlock(&buddy_pte_lock);
				
				spin_lock(&cache_pte_lock);
        			printk("cache pte invocation: %ld\n", cache_pte_cnt);
				cache_pte_cnt=0;
				spin_unlock(&cache_pte_lock);
