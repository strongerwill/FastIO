...
static s_time_t time_base;
static unsigned int time_switch=1;
static unsigned int timing_counter=0;
                           ...
                           int preemptible)
{
	...
	time_base = NOW(); 
      ...
      {
	diff_time = NOW() - time_base;
      	if(diff_time >= SECONDS(60))	
      	//if(diff_time >= SECONDS(5))	
        {
	        local_flush_counter = flush_counter;	
		diff_time = (long long)diff_time/1000000;
		average_flush_counter = (local_flush_counter*60*100*1000)/(long long)diff_time;	
		//average_flush_counter = (flush_counter*5*100*1000)/(long long)diff_time;	
        	...
        	if(timing_counter>=(50*print_counter))
		{
    		...
    		spin_lock(flush_locks);
    	 	//spin_lock_irqsave(&flush_locks, flush_flags);
		flush_counter=0;
    	 	//spin_unlock_irqrestore(&flush_locks, flush_flags);
    		spin_unlock(flush_locks);
		time_switch=1;
	}
      }
    }
    ...
    if ( unlikely((x & PGT_type_mask) != type) )
    {
	    ...
	    if(cache_on)
	    {
            	if ( ((x & PGT_type_mask) == PGT_writable_page) && !(x & PG_prepared))
                	iommu_unmap_page(d, mfn_to_gmfn(d, page_to_mfn(page)));
            	else if ( type == PGT_writable_page )
            	{
			if(((x & PGT_type_mask) == PGT_l3_page_table) || ((x & PGT_type_mask) == PGT_l2_page_table) || ((x & PGT_type_mask) == PGT_l1_page_table))
		//if(((x & PGT_type_mask) == PGT_l3_page_table) || ((x & PGT_type_mask) == PGT_l2_page_table))
		//if((x & PGT_type_mask) == PGT_l1_page_table)
...
/* do_pgd_op */
long do_pgd_op(XEN_GUEST_HANDLE(ptrpgd_t) gptrpg, unsigned int count)
{
    struct vcpu *curr = current;
    struct domain *d = curr->domain;   
    struct page_info *page;
    struct ptrpgd xptrpg;
...
/* do_pmd_op */
long do_pmd_op(XEN_GUEST_HANDLE(ptrpmd_t) gptrpg, unsigned int count)
{
    struct vcpu *curr = current;
    struct domain *d = curr->domain;   
    struct page_info *page;
    struct ptrpmd xptrpg;
    unsigned long pgtable_virt_addr = 0;
    l1_pgentry_t *gl1e;
    l1_pgentry_t xl1e;
    unsigned long gl1mfn;
...
/* do_pte_op */
long do_pte_op(XEN_GUEST_HANDLE(ptrpte_t) gptrpg, unsigned int count)
{
    struct vcpu *curr = current;
    struct domain *d = curr->domain;   
    struct page_info *page;
